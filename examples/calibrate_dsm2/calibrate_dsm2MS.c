...
void *listen_func(void *params){
	...
	for(j=0;j<RC_CHANNELS;j++){
		rc_mins[j]=get_dsm2_ch_rawMS(j+1);
		rc_maxes[j]=get_dsm2_ch_rawMS(j+1);
	}
	...
	while(listening && get_state()!=EXITING){
		...
		if (is_new_dsm2_dataMS()){
			for(j=0;j<RC_CHANNELS;j++){
				if(get_dsm2_ch_rawMS(j+1) > 0){ //record only non-zero channels
					if(get_dsm2_ch_rawMS(j+1)>rc_maxes[j]){
						rc_maxes[j] = get_dsm2_ch_rawMS(j+1);
					}
					else if(get_dsm2_ch_rawMS(j+1)<rc_mins[j]){
						rc_mins[j] = get_dsm2_ch_rawMS(j+1);
					}
					printf(" %d   ",get_dsm2_ch_rawMS(j+1));
				}
			}
			fflush(stdout);
		}
